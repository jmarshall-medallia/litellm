...
class DBSpendUpdateWriter:
    ...
    async def _commit_spend_updates_to_db(  # noqa: PLR0915
        ...
        daily_user_spend_transactions = (
            db_spend_update_transactions["daily_user_spend_transactions"] or {}
        )
        verbose_proxy_logger.debug(
            "Daily User Spend transactions: {}".format(
                len(daily_user_spend_transactions)
            )
        )

        if len(daily_user_spend_transactions) > 0:
            ...
            for i in range(n_retry_times + 1):
                try:
                    ...
                    transactions_to_process = dict(
                        list(daily_user_spend_transactions.items())[:BATCH_SIZE]
                    )
                    ...
                    for key in transactions_to_process.keys():
                        daily_user_spend_transactions.pop(key, None)
